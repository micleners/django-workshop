...
# Django Workshop
## Level 1: API and Template Basics
...
## End of `Master` Branch
You can chose to work forward following two different branches from here - custom auth or no custom auth. You can read more about it below.

- **No Custom Auth**: You can follow this tutorial as is. The work for it is on the branch `no-custom-auth`
- **With Custom Auth**: If you are curious to check out this slightly tedious best practice, checkout the branch `custom-auth`

# 2 (optional) - Create Custom Auth Model

To implement this with the tutorial follow [custom-auth branch](https://github.com/micleners/django-workshop)
...
If you did not create a superuser in Part 2, do so now:
...
Navigate to the `/events/views.py` file in our events app. Create viewsets:
...
In `main_project/urls.py`, register the EventViewSet and the UserViewSet
...
1. Create two new events
2. Create a new user
3. Edit the two new events to have the new user as the presenter
